...
def pvoutputPost(dt, totalWattHour, watt, dcPV) -> str:
    pvoutputString = \
        'data=' + dt.strftime("%Y%m%d") + ',' + dt.strftime("%H:%M") + \
        ','+str(totalWattHour) + ',' + str(watt) + ',-1,-1,,' + str(dcPV)
    log(pvoutputString)
    header = {
    ...
    while True:
        retry = retry + 1
        content = post(PVOUTPUT_ADD_URL, pvoutputString, header)
...
while True:
    ...
    if dateNow.hour < 5 or dateNow.hour > 22:
        log('Outside solar generation hours (5..23)')
        # if you want this program to run forever, comment following line
    ...
    dt = datetime.fromtimestamp(int(dataTimestamp)/1000)
    if dataTimestamp != prevDataTimestamp and dt.day == dateNow.day:
        # only handle new values today
        if prevDataTimestamp != '0':  # check for multiple of 5 minutes
            # round to nearest 5 minutes
            elapsedMinutes = \
                round((int(dataTimestamp) - int(prevDataTimestamp)) / 60000)
            # compute hiResTotalWattHour with current watts/elapsed minutes
            hiResTotalWattHour += int(watt/(60/elapsedMinutes))
            if hiResTotalWattHour < totalWattHour:
                hiResTotalWattHour = totalWattHour  # was too low
            else:
                if totalWattHour + 100 < hiResTotalWattHour:
                    hiResTotalWattHour = totalWattHour + 99  # was too high
